...
The script `automatic_prompting.py` in the `scripts` folder was used to perform requests and record the responses. An Anthropic API key is necessary to execute the script.  
...
The notebook `Validate_Tool_Chains.ipynb` in the `notebooks` folder was used to evaluate the Tool Calling Efficiency metrics. 
...
The notebook `Analyse_Responses.ipynb` in the `notebooks` folder was used to analyse the validated results.
